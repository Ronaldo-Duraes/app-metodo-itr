      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8 mb-8 md:mb-16 border-b border-white/5 pb-8 md:pb-12">
        <div>
          ...
          <h1 className="text-2xl md:text-5xl lg:text-7xl font-black tracking-tighter uppercase leading-none italic">
            Dicionário <span className="text-orange-500">Pessoal</span>
          </h1>
          <button 
            onClick={() => {
              setClearStep(1);
        ...
        <div className="flex flex-row md:flex-col gap-3 md:gap-4 min-w-0 md:min-w-[240px]">
          ...
          <div className="bg-zinc-900/50 border border-white/10 p-4 md:p-6 flex flex-col items-center justify-center flex-1">
            ...
            <div className="text-2xl md:text-4xl font-black text-white leading-none">
              {dictionary.length} <span className="text-sm text-orange-500 uppercase tracking-tighter">palavras</span>
            </div>
          </div>
      ...
      <div className="relative z-[40] -mx-4 md:mx-0 px-4 py-4 md:p-0 bg-black/95 md:bg-transparent backdrop-blur-2xl md:backdrop-blur-none border-b border-white/5 md:border-0 flex flex-col md:flex-row gap-3 md:gap-4 md:mb-12 shadow-none transition-all">
        <div className="relative flex-1 group w-full">
          ...
          <input 
            ...
            className="w-full bg-white/5 border border-white/10 p-4 min-h-[52px] pl-12 text-sm font-bold tracking-widest focus:outline-none focus:border-orange-500/50 transition-all placeholder:text-slate-700 shadow-inner rounded-xl md:rounded-none"
          />
        </div>
        <button 
          ...
          className={`w-full md:w-auto min-h-[52px] px-6 py-4 text-[11px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-3 border rounded-xl md:rounded-none active:scale-[0.98] md:active:scale-100 ${
            sortMode === 'alphabetical' ? 'bg-orange-500/10 border-orange-500/50 text-orange-400' : 
        ...
        <div className="hidden md:grid grid-cols-[50px_1.5fr_1.5fr_120px] px-8 mb-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-700">
          <div className="flex justify-center">
            <div 
              ...
              className={`w-4 h-4 border transition-all cursor-pointer flex items-center justify-center ${selectedIds.length === filteredWords.length && filteredWords.length > 0 ? 'bg-orange-500 border-orange-500' : 'border-white/10 hover:border-white/30'}`}
            >
          ...
          <motion.div 
            ...
            className={`group relative flex flex-col md:grid md:grid-cols-[50px_1.5fr_1.5fr_120px] items-start md:items-center p-3 md:px-8 md:py-5 border transition-all cursor-pointer rounded-xl md:rounded-none w-full md:w-auto mb-2 md:mb-1 ${
              selectedIds.includes(item.id) 
                ? 'bg-orange-500/5 border-orange-500/40 shadow-[0_0_15px_rgba(16,185,129,0.1)]' 
                : 'bg-zinc-900 border-white/5 hover:border-orange-500/30 hover:bg-zinc-800/60'
            }`}
          ...
          >
            ...
            <div className="flex md:hidden justify-between items-center w-full mb-2 pb-2 border-b border-white/5">
              ...
              {item.isMemorized ? (
                ...
                <div className="px-2 py-1 bg-orange-500/10 border border-orange-500/20 text-orange-500 text-[8px] font-black uppercase tracking-tighter rounded-full">
                  Em Revisão
                </div>
              )}
            </div>
            ...
            >
              <div className={`w-5 h-5 border-2 transition-all flex items-center justify-center ${
                selectedIds.includes(item.id) 
                  ? 'border-orange-500 bg-orange-500 shadow-[0_0_10px_rgba(16,185,129,0.3)]' 
            ...
            <div className="flex w-full md:contents items-center justify-between gap-2">
              <div className="grid grid-cols-2 gap-2 flex-1 md:contents items-center">
                ...
                <div className="min-w-0 pr-1 md:pr-0">
                  <h3 className="text-xs md:text-base font-black text-white uppercase tracking-tight group-hover:text-orange-400 transition-colors truncate">
                    {item.translation}
                  </h3>
                </div>
                ...
                <div className="min-w-0 border-l border-white/10 pl-2 md:border-0 md:pl-0">
                  <p className="text-xs md:text-sm text-orange-500 md:text-slate-500 font-bold uppercase tracking-tight md:tracking-widest font-mono truncate">
                    {item.word}
                  </p>
                </div>
              </div>
            ...
            <div className="hidden w-full md:w-auto md:flex justify-end items-center gap-4 relative">
              <div className="hidden md:block">
                {item.isMemorized ? (
                  ...
                  <div className="px-3 py-1 bg-orange-500/10 border border-orange-500/20 text-orange-500 text-[9px] font-black uppercase tracking-tighter">
                    Em Revisão
                  </div>
                )}
              </div>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              <div className="flex justify-between items-center mb-8">
                <h2 className="text-2xl font-black uppercase tracking-tighter italic">Editar <span className="text-orange-500">Termo</span></h2>
              ...
              <div className="space-y-6">
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-orange-500 mb-2 block">Português (Destaque)</label>
                  <input 
                    type="text" 
                    value={editForm.translation}
                    onChange={(e) => setEditForm(prev => ({ ...prev, translation: e.target.value }))}
                    className="w-full bg-white/5 border border-white/10 p-4 text-sm font-bold uppercase tracking-widest focus:outline-none focus:border-orange-500 transition-all"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 mb-2 block">Espanhol</label>
                  <input 
                    type="text" 
                    value={editForm.word}
                    onChange={(e) => setEditForm(prev => ({ ...prev, word: e.target.value }))}
                    className="w-full bg-white/5 border border-white/10 p-4 text-sm font-bold uppercase tracking-widest focus:outline-none focus:border-orange-500 transition-all font-mono"
                  />
                </div>
                
                <button 
                  onClick={handleSaveEdit}
                  className="w-full bg-orange-500 hover:bg-orange-400 text-black p-4 text-xs font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2 mt-4"
                >
      ...
      <AnimatePresence>
        {selectedIds.length > 0 && (
          <motion.div 
            ...
            className="fixed bottom-6 left-1/2 z-[100] bg-zinc-950 border-2 border-orange-500/30 px-4 md:px-8 py-4 md:py-5 flex flex-col sm:flex-row items-center gap-4 md:gap-10 shadow-[0_0_50px_rgba(0,0,0,0.8)] w-[calc(100%-2rem)] sm:w-auto"
          >
            <div className="flex items-center gap-4 border-r border-white/10 pr-10">
              <div className="w-2 h-2 rounded-full bg-orange-500 animate-pulse" />
              <span className="text-xs font-black text-white uppercase tracking-[0.2em]">
                {selectedIds.length} <span className="text-orange-500">Selecionados</span>
              </span>
            </div>
      ...
      <AnimatePresence>
        {showSuccessToast && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: 20 }}
            className="fixed bottom-6 left-1/2 md:right-10 z-[250] bg-orange-500 text-black px-6 py-3 md:px-8 md:py-4 font-black text-xs uppercase tracking-widest shadow-2xl flex items-center gap-3 -translate-x-1/2 md:translate-x-0 md:left-auto w-[calc(100%-3rem)] md:w-auto"
          >
