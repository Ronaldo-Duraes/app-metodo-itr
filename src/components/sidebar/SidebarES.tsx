...
const Sidebar = ({ isOpen = false, onClose }: SidebarProps) => {
  ...
  const handleProfileClick = (e: React.MouseEvent) => {
    e.preventDefault();
    onClose?.();
    router.push('/espanhol/perfil?t=' + Date.now());
  };
  ...
  const menuItems = [
    { icon: Home, label: 'Home', path: '/espanhol' },
    { icon: Zap, label: 'Atividades', path: '/espanhol/atividades', id: 'tour-atividades' },
    { icon: BookOpen, label: 'Flashcards', path: '/espanhol/flashcards', id: 'tour-flashcards' },
    { icon: GraduationCap, label: 'Acessar Aulas', path: '/espanhol/aulas', id: 'tour-aulas' },
    { icon: Library, label: 'Dicionário Pessoal', path: '/espanhol/dicionario', id: 'tour-dicionario' },
  ...
  const MobileProfileHeader = () => (
    <div 
      ...
      className="mx-4 mb-6 p-4 bg-white/[0.02] border border-white/5 cursor-pointer group hover:border-orange-500/20 transition-all relative overflow-hidden"
    >
      {/* Subtle glow */}
      <div className="absolute top-0 right-0 w-24 h-24 bg-orange-500/5 -translate-y-1/2 translate-x-1/2 rounded-full blur-[30px] group-hover:scale-150 transition-transform duration-500" />
      
      <div className="relative flex items-center gap-3">
        {/* Avatar */}
        <div className="relative shrink-0">
          <div className="w-12 h-12 rounded-full bg-zinc-900 border-2 border-white/10 overflow-hidden flex items-center justify-center group-hover:border-orange-500/40 transition-colors">
            {isVisitor ? (
          ...
          {/* Online Status */}
          {!isVisitor && (
            <div className="absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 bg-orange-500 border-2 border-[#050505] rounded-full" />
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[9px] font-black text-orange-500/70 tracking-[0.15em] uppercase leading-none">
            {roleLabel}
          </div>
        </div>
      ...
      <div className="mt-3 pt-3 border-t border-white/5">
        <div className="flex items-center justify-between mb-1.5">
          <div className="flex items-center gap-1.5">
            <PatenteIcon size={12} className="text-orange-500" />
            <span className="text-[8px] font-black text-zinc-500 uppercase tracking-[0.2em]">
              {patenteName}
            </span>
          </div>
          <span className="text-[8px] font-black text-orange-500/60 tracking-widest">
  ...
  const desktopSidebarContent = (
    <div className="flex flex-col h-full py-8">
          ...
          if (item.isAction) {
            return (
              <div 
                ...
                className="group relative flex items-center gap-4 px-4 py-4 min-h-[52px] rounded-none transition-all duration-200 cursor-pointer text-orange-500/50 hover:text-orange-400 hover:bg-orange-500/5 border-l-4 border-transparent hover:border-orange-500"
              >
          ...
          return (
            <Link key={item.path} href={item.path} onClick={handleNavClick}>
              <div id={item.id} className={`
                ...
                ${isActive 
                  ? 'bg-white/[0.03] border-l-4 border-orange-500 text-white shadow-[inset_10px_0_20px_rgba(16,185,129,0.02)]' 
                  : 'text-slate-500 hover:text-white hover:bg-white/[0.02] border-l-4 border-transparent'}
              `}>
                <Icon 
                  size={20} 
                  className={`transition-colors duration-200 shrink-0 ${isActive ? 'text-orange-400' : 'group-hover:text-slate-300'}`} 
                ...
                {isActive && (
                  <div className="absolute inset-0 bg-orange-500/[0.01] blur-2xl -z-10" />
                )}
              </div>
            </Link>
          );
      ...
      <div className="px-4 mt-auto pt-8 border-t border-white/5">
        <Link href="/espanhol/perfil" onClick={handleProfileClick}>
          <div id="tour-perfil" className="flex items-center gap-3 p-3 min-h-[52px] text-slate-500 hover:text-white transition-all cursor-pointer group bg-transparent hover:bg-white/[0.03]">
            <div className="relative shrink-0">
              <div className="w-10 h-10 rounded-full bg-slate-900 border border-white/10 overflow-hidden flex items-center justify-center shrink-0 group-hover:border-orange-500/50 transition-colors">
                {isVisitor ? (
              ...
              {/* Online Status */}
              <div className="absolute bottom-0 right-0 w-3 h-3 bg-orange-500 border-2 border-[#050505] rounded-full"></div>
            </div>
            ...
            <div className="flex flex-col justify-center overflow-hidden">
              ...
              <span className="text-[9px] font-black text-orange-500/70 tracking-[0.2em] uppercase leading-none truncate">
                {roleLabel}
              </span>
            </div>
          </div>
          ...
          if (item.isAction) {
            return (
              <div 
                ...
                className="group relative flex items-center gap-4 px-4 py-4 min-h-[52px] rounded-none transition-all duration-200 cursor-pointer text-orange-500/50 hover:text-orange-400 hover:bg-orange-500/5 border-l-4 border-transparent hover:border-orange-500 active:bg-orange-500/10"
              >
