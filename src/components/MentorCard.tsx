  ...
  return (
    ...
    >
      {isModal && onClose && (
        <button 
          onClick={onClose}
          className="absolute top-6 right-6 z-[60] p-2 bg-white/5 hover:bg-white/10 rounded-full border border-white/10 text-white/50 hover:text-white transition-all shadow-lg backdrop-blur-md"
        >
          <X size={20} />
        </button>
      )}

      <div className={`flex ${!isModal ? 'flex-col md:flex-row' : 'flex-row'} items-stretch`}>
        ...
        <div className={`relative ${isModal ? 'w-[40%] aspect-[4/5]' : 'w-full md:w-1/3 h-72 md:h-auto aspect-square md:aspect-auto'} overflow-hidden`}>
          <Image
            ...
            sizes={isModal ? "500px" : "(max-width: 768px) 100vw, 33vw"}
          />
        ...
        <div className={`flex-1 ${isModal ? 'p-8' : 'p-8 md:p-12'} flex flex-col justify-center gap-6 bg-gradient-to-br from-[#0a0a0a] via-[#0d0d0d] to-[#0a0a0a]`}>
          ...
          {/* Glass Style Buttons */}
          <div className={`flex flex-wrap ${isModal ? 'flex-row gap-4' : 'gap-4'} mt-2`}>
            <motion.a
            ...
            <div className={`flex items-center justify-center gap-2 ${isModal ? 'px-6 py-3 text-xs' : 'px-8 py-4'} bg-white/5 backdrop-blur-sm border border-white/5 rounded-2xl text-slate-500 cursor-not-allowed transition-all duration-300 shadow-none opacity-60`}>
              <MessageCircle size={isModal ? 16 : 20} className="text-slate-600" />
